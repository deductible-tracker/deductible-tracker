...
import { clearCurrentUser, getCurrentUser, getCurrentUserId, setCurrentUser } from './services/current-user.js';
...
import { renderCharitiesRoute, renderCharityEditRoute, renderCharityNewRoute } from './views/routes/charities.js';
...
function updateActiveLink(path) {
    ...
    document.querySelectorAll('a[data-route]').forEach(a => {
        if (a.dataset.route === navRoute) {
            ...
            a.classList.remove('text-slate-700', 'dark:text-slate-300', 'hover:bg-indigo-50', 'hover:text-indigo-700', 'dark:hover:bg-slate-800', 'dark:hover:text-indigo-400');
        } else {
            a.classList.remove('bg-indigo-600', 'text-white', 'dark:bg-indigo-500');
            a.classList.add('text-slate-700', 'dark:text-slate-300', 'hover:bg-indigo-50', 'hover:text-indigo-700', 'dark:hover:bg-slate-800', 'dark:hover:text-indigo-400');
        }
    });
}
...
async function openReceiptManager(donationId) {
    await navigate(`/donations/receipts/${donationId}`);
}
