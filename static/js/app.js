...
let pendingDonationCategory = null;
let pendingCharityEditId = null;
...
async function clearUserCaches() {
    try { await db.donations.clear(); } catch (e) { /* ignore */ }
    try { await db.receipts.clear(); } catch (e) { /* ignore */ }
    try { await db.charities.clear(); } catch (e) { /* ignore */ }
...
async function createOrGetCharityOnServer(nameOrPayload, ein) {
...
async function lookupCharityByEinOnServer(ein) {
...
async function updateCharityOnServer(charityId, payload) {
...
async function fetchCharitiesFromServer() {
    const { res, data } = await apiJson('/api/charities');
    if (!res.ok) {
...
async function deleteCharityOnServer(charityId) {
    const { res, data } = await apiJson(`/api/charities/${encodeURIComponent(charityId)}`, {
        method: 'DELETE'
    });
    if (res.status === 409) {
...
async function createDonationOnServer(payload) {
...
async function deleteDonationOnServer(donationId) {
...
async function navigate(path, options = {}) {
    ...
    const handler = routes[target] || routes['/'];
    await handler();
...
function updateActiveLink(path) {
    document.querySelectorAll('nav a').forEach(a => {
        if (a.dataset.route === path) {
            ...
            a.classList.remove('text-slate-700', 'hover:bg-slate-100');
        } else {
...
async function handleLogout() {
    ...
    await clearUserCaches();
    try { localStorage.removeItem(profileKey); } catch (e) { /* ignore */ }
    clearCurrentUser();
...
async function renderDashboard() {
    ...
    const btnAddInlineHome = document.getElementById('btn-add-donation-inline-home');
    if (btnAddInlineHome) btnAddInlineHome.addEventListener('click', () => openAddDonationModal());
...
async function renderRecentList() {
    ...
    if (donations.length === 0) {
        list.innerHTML = '<li class="px-5 py-4 text-sm text-slate-500">No donations yet.</li>';
        return;
    }
...
async function renderDonations() {
    ...
    const donations = userId
        ? (await db.donations.where('user_id').equals(userId).toArray())
            .sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')))
        : [];
    const receipts = await db.receipts.toArray();
    const taxEstimates = await calculateTaxEstimates(donations, charities, receipts);
    const categoryPrefill = pendingDonationCategory && ['items', 'money', 'mileage'].includes(pendingDonationCategory)
        ? pendingDonationCategory
        : 'money';
    pendingDonationCategory = null;
    ...
    charityInput?.addEventListener('input', (e) => {
        ...
        suggestionTimer = setTimeout(async () => {
            try {
                ...
                let remote = [];
                try {
                    const res = await fetch(`/api/charities/search?q=${encodeURIComponent(q)}`, { credentials: 'include' });
                    if (res.ok) {
                ...
                for (const c of localMatches) {
                    ...
                    if (!seen.has(key)) {
                        seen.add(key);
                        merged.push({ id: c.id, ein: c.ein || '', name: c.name, location: '', source: 'local' });
                    }
                }
                for (const r of remote) {
                    ...
                    if (!seen.has(key)) {
                        seen.add(key);
                        merged.push({ id: '', ein: r.ein || '', name: r.name, location: r.location || '', source: 'remote' });
                    }
                }

                if (merged.length === 0) {
                    suggestionsBox.innerHTML = '<div class="p-2 text-sm text-slate-500">No matches</div>';
                    suggestionsBox.classList.remove('hidden');
                    return;
                }

                suggestionsBox.innerHTML = merged.map(r => `
                ...
                suggestionsBox.querySelectorAll('div[data-id][data-ein][data-name]').forEach(el => {
                    el.addEventListener('click', () => {
                        ...
                        charityInput.value = name;
                        charityIdInput.value = id;
                        charityEinInput.value = ein;
        ...
        try {
            ...
            if (receiptFiles.length > 0 && donation.sync_status === 'synced') {
                for (const file of receiptFiles) {
                    await uploadReceiptForDonation(file, donation.id);
                }
            } else if (receiptFiles.length > 0 && donation.sync_status !== 'synced') {
...
async function openReceiptManager(donationId) {
    // Create modal
    const modal = document.createElement('div');
    modal.className = 'dt-modal-wrap';
    modal.innerHTML = `
    ...
    async function refreshLists() {
        const normalizedDonationId = (donationId && String(donationId).trim()) ? String(donationId) : null;
        if (!normalizedDonationId) {
            return;
        }

        const attached = await db.receipts.where('donation_id').equals(normalizedDonationId).toArray();

        const attachedList = modal.querySelector('#attached-list');

        attachedList.innerHTML = attached.length === 0 ? '<div class="dt-panel-muted px-3 py-2 text-sm text-slate-500">No attached receipts.</div>' : attached.map(r => `
        ...
        modal.querySelectorAll('.preview-receipt-btn').forEach(b => {
            b.addEventListener('click', async (e) => {
                const key = e.currentTarget.dataset.key;
                try {
                    const downloadUrl = await getReceiptDownloadUrl(key);
    ...
    try {
        ...
        alert('Unable to load receipts right now. Please try again.');
    }
}

async function getReceiptDownloadUrl(key) {
...
// --- Add Donation Modal ---
async function openAddDonationModal(initialCategory = null) {
    if (initialCategory && ['items', 'money', 'mileage'].includes(initialCategory)) {
        pendingDonationCategory = initialCategory;
    }
...
async function openAddCharityModal(existingCharity = null) {
    pendingCharityEditId = existingCharity && existingCharity.id ? existingCharity.id : null;
    await navigate('/charities');
    const form = document.getElementById('charity-page-form');
    if (form) {
        form.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
}
...
async function renderReports() {
    ...
    try {
        ...
        if (res.ok && data && Array.isArray(data.years)) {
            years = data.years
                .filter(y => Number.isInteger(y))
                .sort((a, b) => b - a);
        }
    } catch (e) {
        console.warn('Failed to load report years', e);
    }

    const hasDonationYears = years.length > 0;
    ...
    const taxTxfBtn = document.getElementById('btn-export-tax-txf');

    if (!hasDonationYears) {
        csvBtn.disabled = true;
        taxTxfBtn.disabled = true;
    }

    const downloadReport = async (endpoint, extension) => {
        ...
        try {
            ...
            a.href = url;
            a.download = `donations-${year}.${extension}`;
    ...
    csvBtn.addEventListener('click', async () => {
        await downloadReport('/api/reports/export', 'csv');
    });

    taxTxfBtn.addEventListener('click', async () => {
        await downloadReport('/api/reports/export/txf', 'txf');
    });
}
...
async function renderCharities() {
    ...
    const formatAddress = (charity) => {
        const parts = [charity.street, charity.city, charity.state, charity.zip]
            .map(v => (v || '').trim())
            .filter(Boolean);
        return parts.length ? parts.join(', ') : '—';
    };

    const existing = pendingCharityEditId ? charities.find(c => c.id === pendingCharityEditId) : null;
    ...
    document.getElementById('charity-page-form').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (isEditMode && existing?.id) {
                ...
                const resp = await updateCharityOnServer(existing.id, updatePayload);
                charity = resp && resp.charity ? resp.charity : { id: existing.id, ...updatePayload };
            } else {
    ...
    const cancelEditBtn = document.getElementById('btn-cancel-charity-edit');
    if (cancelEditBtn) {
    ...
    document.querySelectorAll('.edit-charity-btn').forEach(button => {
        button.addEventListener('click', async (e) => {
            pendingCharityEditId = e.currentTarget.dataset.id || null;
            await renderCharities();
        });
    });
...
async function renderPersonalInfo() {
    ...
    document.getElementById('personal-form').addEventListener('submit', async (e) => {
        ...
        const profile = {
            ...
            tax_id: document.getElementById('profile-tax').value.trim(),
            filing_status: document.getElementById('profile-filing-status').value,
            agi: parseFloat(document.getElementById('profile-agi').value || ''),
            marginal_tax_rate: parseFloat(document.getElementById('profile-marginal-rate').value || ''),
            itemize_deductions: document.getElementById('profile-itemize').checked,
        };
        if (!Number.isFinite(profile.agi)) profile.agi = null;
        if (!Number.isFinite(profile.marginal_tax_rate)) profile.marginal_tax_rate = null;
        try {
            const { res, data } = await apiJson('/api/me', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
...
function normalizeDonationCategory(category) {
    if (!category) return 'money';
    const normalized = String(category).toLowerCase();
    if (normalized === 'items' || normalized === 'money' || normalized === 'mileage') {
        return normalized;
    }
    return 'money';
}
...
function emptyFigure() {
    return { amount: 0, count: 0, hasAmount: false };
}

function calculateDonationFigures(donations) {
    const figures = {
        total: emptyFigure(),
        items: emptyFigure(),
        money: emptyFigure(),
    ...
    for (const donation of donations || []) {
        ...
        const bucket = figures[category] || figures.money;
        const amount = parseAmount(donation.amount);

        figures.total.count += 1;
        bucket.count += 1;

        if (amount !== null) {
            figures.total.amount += amount;
            figures.total.hasAmount = true;
...
function formatFigureText(figure) {
    if (!figure) return '$0.00';
    if (figure.hasAmount) return `$${figure.amount.toFixed(2)}`;
    if (figure.count > 0) return String(figure.count);
    return '$0.00';
}
...
function isLikelyQualifiedCharity(charity) {
...
async function calculateTaxEstimates(donations, charities, receipts) {
...
async function uploadReceiptForDonation(file, donationId) {
...
async function updateTotals() {
    try {
        ...
        const itemsEl = document.getElementById('items-amount');
        const moneyEl = document.getElementById('money-amount');
        const mileageEl = document.getElementById('mileage-amount');
        const charitiesCountEl = document.getElementById('charities-count');
        ...
        if (charitiesCountEl) {
            const count = userId ? await db.charities.where('user_id').equals(userId).count() : 0;
            charitiesCountEl.textContent = `${count} charities`;
        }
    } catch (e) {
...
async function updateSyncStatus() {
    ...
    let pending = 0;
    try {
        if (userId) pending = await db.sync_queue.where('user_id').equals(userId).count();
    } catch (e) { /* ignore */ }
...
async function init() {
    ...
    try {
        ...
        if (isSchemaMismatch) {
            ...
            if (alreadyReset) {
                console.error('Dexie schema reset already attempted for this session; aborting retry loop.', e);
                throw e;
            }

            console.warn('Dexie schema mismatch detected. Clearing local database and reloading.');
            sessionStorage.setItem(schemaResetKey, '1');
            try { await db.delete(); } catch (de) { /* ignore */ }
    ...
    try { sessionStorage.removeItem('dexie_schema_reset_done'); } catch (_) { /* ignore */ }
    ...
    if (btnAddDonation) {
        btnAddDonation.addEventListener('click', () => openAddDonationModal());
    }
    ...
    if (btnAddDonationMobile) {
        btnAddDonationMobile.addEventListener('click', async () => {
            ...
            await openAddDonationModal();
        });
    }

    const btnAddDonationTop = document.getElementById('btn-add-donation-top');
    if (btnAddDonationTop) {
        btnAddDonationTop.addEventListener('click', () => openAddDonationModal());
    }

    const btnItemsAdd = document.getElementById('btn-items-add');
    if (btnItemsAdd) btnItemsAdd.addEventListener('click', () => openAddDonationModal('items'));
    const btnMoneyAdd = document.getElementById('btn-money-add');
    if (btnMoneyAdd) btnMoneyAdd.addEventListener('click', () => openAddDonationModal('money'));
    const btnMileageAdd = document.getElementById('btn-mileage-add');
    if (btnMileageAdd) btnMileageAdd.addEventListener('click', () => openAddDonationModal('mileage'));

    const btnItemsView = document.getElementById('btn-items-view');
    if (btnItemsView) btnItemsView.addEventListener('click', async () => navigate('/donations'));
    const btnMoneyView = document.getElementById('btn-money-view');
    if (btnMoneyView) btnMoneyView.addEventListener('click', async () => navigate('/donations'));
    const btnMileageView = document.getElementById('btn-mileage-view');
    if (btnMileageView) btnMileageView.addEventListener('click', async () => navigate('/donations'));

    const btnCharityAddDashboard = document.getElementById('btn-charity-add-dashboard');
    if (btnCharityAddDashboard) btnCharityAddDashboard.addEventListener('click', () => openAddCharityModal());
    const btnCharityViewDashboard = document.getElementById('btn-charity-view-dashboard');
    if (btnCharityViewDashboard) btnCharityViewDashboard.addEventListener('click', async () => navigate('/charities'));

    const btnUploadReceiptMobile = document.getElementById('btn-upload-receipt-mobile');
    if (btnUploadReceiptMobile) {
        btnUploadReceiptMobile.addEventListener('click', () => {
            const mobile = document.getElementById('mobile-menu');
            if (mobile) mobile.classList.add('hidden');
            const desktopUploadBtn = document.getElementById('btn-upload-receipt');
            if (desktopUploadBtn) desktopUploadBtn.click();
        });
    }
