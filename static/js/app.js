    ...
    if (configRes.ok) {
      ...
      if (config.google_enabled && config.google_client_id) {
        try {
          ...
          // If running on localhost, use a local redirect URI
          if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') {
            gIdOnload.setAttribute('data-login_uri', `${location.origin}/auth/callback/google`);
          } else if (!gIdOnload.getAttribute('data-login_uri')) {
            // Preserve any existing data-login_uri if provided; otherwise default
            gIdOnload.setAttribute('data-login_uri', `/auth/callback/google`);
          }
