export async function renderDonationsRoute(deps) {
    ...
    const taxEstimates = await calculateTaxEstimates(donations, charities, receipts, getCurrentUser() || {});
    ...
    document.getElementById('btn-new-donation')?.addEventListener('click', () => navigate('/donations/new'));
...
async function getReceiptDownloadUrlRoute(key) {
...
function buildDonationFormHtmlRoute({ title, desc, submitLabel, categoryPrefill = 'money', existing = null }, deps) {
...
async function bindDonationFormHandlersRoute({ userId, charities, existingDonation }, deps) {
            ...
            try {
                ...
                try {
                    localMatches = await db.charities
                        ...
                        .toArray();
                } catch (le) { console.warn('Local charity lookup failed', le); }

                let remote = [];
                try {
                    const res = await fetch(`/api/charities/search?q=${encodeURIComponent(q)}`, { credentials: 'include' });
                    if (res.ok) { const data = await res.json(); remote = data.results || []; }
                } catch (re) { console.warn('Remote charity search failed', re); }

                const seen = new Set();
                const merged = [];
                for (const c of localMatches) {
                    const key = ((c.ein || '').trim() || (c.name || '').trim()).toLowerCase();
                    if (!seen.has(key)) { seen.add(key); merged.push({ id: c.id, ein: c.ein || '', name: c.name, location: '', source: 'local' }); }
                }
                for (const r of remote) {
                    const key = ((r.ein || '').trim() || (r.name || '').trim()).toLowerCase();
                    if (!seen.has(key)) { seen.add(key); merged.push({ id: '', ein: r.ein || '', name: r.name, location: r.location || '', source: 'remote' }); }
                }

                if (merged.length === 0) {
                    suggestionsBox.innerHTML = '<div class="p-2 text-sm text-slate-500 dark:text-slate-400">No matches</div>';
    ...
    document.getElementById('donation-page-form')?.addEventListener('submit', async (e) => {
        ...
        if (!date || !charity_name) { alert('Please provide date and charity name'); return; }
        ...
        try {
            ...
            const payload = { date, charity_name, charity_id: charityId || null, charity_ein: charity_ein || null, category, amount, notes: notes || null };
            ...
            if (isEditMode) {
                ...
                donation = { ...existingDonation, date, charity_id: charityId || existingDonation.charity_id, notes: notes || null, category, amount, sync_status: 'synced' };
                await db.donations.put(donation);
            } else {
                try {
                    const res = await createDonationOnServer(payload);
                    const serverId = res && res.id ? res.id : fallbackId;
                    donation = { id: serverId, user_id: userId, year, date, charity_id: charityId || null, notes: notes || null, category, amount, sync_status: 'synced' };
                    await db.donations.put(donation);
                } catch {
                    donation = { id: fallbackId, user_id: userId, year, date, charity_id: charityId || null, notes: notes || null, category, amount, sync_status: 'pending' };
                    await Sync.queueAction('donations', donation, 'create');
                }
            }

            if (receiptFiles.length > 0 && donation.sync_status === 'synced') {
                for (const file of receiptFiles) { await uploadReceiptForDonationRoute(file, donation.id, deps); }
...
export async function renderDonationNewRoute(deps) {
    ...
    root.innerHTML = buildDonationFormHtmlRoute({ title: 'New Donation', desc: 'Add a donation and optionally attach receipts.', submitLabel: 'Save Donation', categoryPrefill: 'money' }, deps);
...
export async function renderDonationEditRoute(donationId, deps) {
    ...
    if (!existing) { alert('Donation not found'); await deps.navigate('/donations'); return; }
...
export async function renderReceiptPageRoute(donationId, deps) {
    ...
    document.getElementById('btn-back-donations')?.addEventListener('click', () => deps.navigate('/donations'));
    ...
    document.getElementById('btn-upload-receipts')?.addEventListener('click', async () => {
        ...
        try {
            for (const file of files) { await uploadReceiptForDonationRoute(file, donationId, deps); }
            fileInput.value = '';
...
async function refreshAttachedListRoute(donationId, container, deps) {
    ...
    attachedList.innerHTML = attached.length === 0
    ...
    attachedList.querySelectorAll('.preview-receipt-btn').forEach(b => {
        b.addEventListener('click', async (e) => {
            if (!navigator.onLine) { alert('Receipt preview requires an internet connection.'); return; }
            const key = e.currentTarget.dataset.key;
            try {
                const downloadUrl = await getReceiptDownloadUrlRoute(key);
