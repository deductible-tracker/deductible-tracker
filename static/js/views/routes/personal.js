...
function renderRateOptionsRoute(brackets, currentRate) {
    if (!brackets || brackets.length === 0) {
        const defaults = [0.10, 0.12, 0.22, 0.24, 0.32, 0.35, 0.37];
...
export async function renderPersonalInfoRoute(deps) {
    ...
    if (navigator.onLine) {
        ...
        } catch (e) { console.warn('Failed to load profile from server', e); }
    }

    let initialRateData = { brackets: [], selectedRate: null };
    if (navigator.onLine) {
        try {
            initialRateData = await fetchMarginalRateDataRoute(profile.filing_status, Number(profile.agi), deps);
            if (initialRateData.selectedRate !== null) profile.marginal_tax_rate = initialRateData.selectedRate;
        } catch (e) { console.warn('Failed to fetch initial marginal tax rate', e); }
    }
    ...
    const syncMarginalRateFromServer = async () => {
        ...
        } catch (e) { console.warn('Failed to sync marginal tax rate', e); }
    };
    ...
    document.getElementById('personal-form').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            await deps.Sync.queueProfileUpdate(deps.getCurrentUserId());
