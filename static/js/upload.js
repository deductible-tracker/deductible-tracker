...
function getToastHost() {
    ...
    host.id = 'toast-host';
    host.className = 'fixed top-20 right-4 z-50 flex w-[22rem] max-w-[calc(100vw-2rem)] flex-col gap-2 pointer-events-none';
...
async function chooseDonationForUpload() {
    ...
    const donations = (await db.donations.where('user_id').equals(userId).toArray())
    ...
    const choices = shortlist.map((d, index) => (
