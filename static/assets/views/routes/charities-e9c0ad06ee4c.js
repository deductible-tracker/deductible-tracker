export async function renderCharitiesRoute(deps){const{db,deleteCharityOnServer,escapeHtml,getCurrentUserId,navigate,refreshCharitiesCache}=deps;const root=document.getElementById('route-content')||document.getElementById('app');const userId=getCurrentUserId();let charities=[];try{if(userId){charities=await refreshCharitiesCache();}}catch{charities=userId?await db.charities.where('user_id').equals(userId).toArray():[];}const formatAddress=(c)=>{const parts=[c.street,c.city,c.state,c.zip].map(v=>(v||'').trim()).filter(Boolean);return parts.length?parts.join(', '):'—';};root.innerHTML=`
    ...
    `;document.getElementById('btn-new-charity')?.addEventListener('click',()=>navigate('/charities/new'));document.querySelectorAll('.edit-charity-btn').forEach(button=>{button.addEventListener('click',(e)=>{navigate(`/charities/edit/${encodeURIComponent(e.currentTarget.dataset.id)}`);});});document.querySelectorAll('.delete-charity-btn').forEach(b=>{b.addEventListener('click',async(e)=>{const charityId=e.currentTarget.dataset.id;if(!confirm('Delete cached charity?'))return;try{const uid=getCurrentUserId();if(uid&&charityId){await deleteCharityOnServer(charityId);await db.charities.delete(charityId);}await renderCharitiesRoute(deps);}catch(err){console.error(err);alert(err.message||'Failed to delete');}});});}function buildCharityFormHtmlRoute({title,isEditMode,existing},deps){const{escapeHtml}=deps;const e=existing||{};return`
    ...
    `;}async function bindCharityFormHandlersRoute({userId,existingCharity,isEditMode},deps){const{apiJson,createOrGetCharityOnServer,db,escapeHtml,lookupCharityByEinOnServer,navigate,updateCharityOnServer,updateTotals,}=deps;const norm=(value)=>{if(value===null||value===undefined)return null;const trimmed=String(value).trim();return trimmed?trimmed:null;};document.getElementById('btn-back-charities')?.addEventListener('click',()=>navigate('/charities'));const nameInput=document.getElementById('charity-name');const einInput=document.getElementById('charity-ein');const nonprofitTypeInput=document.getElementById('charity-nonprofit-type');const categoryInput=document.getElementById('charity-category');const statusInput=document.getElementById('charity-status');const classificationInput=document.getElementById('charity-classification');const deductibilityInput=document.getElementById('charity-deductibility');const streetInput=document.getElementById('charity-street');const cityInput=document.getElementById('charity-city');const stateInput=document.getElementById('charity-state');const zipInput=document.getElementById('charity-zip');const suggestions=document.getElementById('charity-name-suggestions');if(!isEditMode&&nameInput){let searchTimer=null;nameInput.addEventListener('input',()=>{const query=nameInput.value.trim();if(searchTimer)clearTimeout(searchTimer);if(!query||query.length<2){suggestions.innerHTML='';suggestions.classList.add('hidden');return;}searchTimer=setTimeout(async()=>{try{const{res,data}=await apiJson(`/api/charities/search?q=${encodeURIComponent(query)}`);if(!res.ok||!data||!Array.isArray(data.results)||data.results.length===0){suggestions.innerHTML='';suggestions.classList.add('hidden');return;}suggestions.innerHTML=data.results.slice(0,7).map(item=>`
