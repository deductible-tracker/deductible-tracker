export async function renderDashboardRoute(deps){const{renderRecentList,}=deps;const root=document.getElementById('route-content')||document.getElementById('app');root.innerHTML=`
    ...
    `;if(window.lucide)lucide.createIcons();renderRecentList();}export async function renderRecentListRoute(deps){const{db,escapeHtml,getCurrentUserId,getUserCharityNameMap}=deps;const list=document.getElementById('recent-list');const userId=getCurrentUserId();const charityNameMap=await getUserCharityNameMap();const donations=(await db.donations.orderBy('date').reverse().toArray()).filter(d=>d.user_id===userId).slice(0,5);if(donations.length===0){list.innerHTML='<li class="px-5 py-4 text-sm text-slate-500 dark:text-slate-400">No donations yet.</li>';return;}list.innerHTML=donations.map(d=>`
