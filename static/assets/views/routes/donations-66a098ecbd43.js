export async function renderDonationsRoute(deps){const{calculateTaxEstimates,createDonationOnServer,createOrGetCharityOnServer,db,deleteDonationOnServer,escapeHtml,formatCurrency,getCurrentUser,getCurrentUserId,isCharityCacheFresh,navigate,Sync,updateDonationOnServer,updateTotals,}=deps;const root=document.getElementById('route-content')||document.getElementById('app');const userId=getCurrentUserId();const charities=userId?await db.charities.where('user_id').equals(userId).toArray():[];const charityNameMap=new Map(charities.map(c=>[c.id,c.name||'Unknown charity']));const donations=userId?(await db.donations.where('user_id').equals(userId).toArray()).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))):[];const receipts=await db.receipts.toArray();const taxEstimates=await calculateTaxEstimates(donations,charities,receipts,getCurrentUser()||{});root.innerHTML=`
        <div class="mx-auto max-w-7xl space-y-5">
            ...
            <div class="dt-panel overflow-hidden">
                ...
                <div class="space-y-3 p-4 md:hidden">
                    ${donations.length === 0 ? '<div class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 text-sm text-slate-500 dark:text-slate-400">No donations yet.</div>' : donations.map(d => `
    ...
    `;document.getElementById('btn-new-donation')?.addEventListener('click',()=>navigate('/donations/new'));document.querySelectorAll('.edit-donation-btn').forEach(btn=>{btn.addEventListener('click',(e)=>{navigate(`/donations/edit/${encodeURIComponent(e.currentTarget.dataset.id)}`);});});document.querySelectorAll('.manage-receipts-btn').forEach(btn=>{btn.addEventListener('click',(e)=>{const id=e.currentTarget.dataset.id;openReceiptManagerRoute(id,deps);});});document.querySelectorAll('.delete-donation-btn').forEach(btn=>{btn.addEventListener('click',async(e)=>{const id=e.currentTarget.dataset.id;if(!confirm('Delete this donation? Associated receipts will also be removed.'))return;try{await deleteDonationOnServer(id);const localReceipts=await db.receipts.where('donation_id').equals(id).toArray();for(const r of localReceipts){await db.receipts.delete(r.id);}await db.donations.delete(id);await renderDonationsRoute(deps);await updateTotals();}catch(err){console.error('Delete failed',err);alert('Failed to delete donation');}});});}export async function openReceiptManagerRoute(donationId,deps){await deps.navigate(`/donations/receipts/${donationId}`);}async function getReceiptDownloadUrlRoute(key){const res=await fetch('/api/receipts/presign',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({key})});if(!res.ok)throw new Error('Presign failed');const data=await res.json();return data.download_url;}function buildDonationFormHtmlRoute({title,desc,submitLabel,categoryPrefill='money',existing=null},deps){const{escapeHtml}=deps;const d=existing||{};return`
            ...
            <div class="dt-panel p-5 sm:p-6">
                <form id="donation-page-form" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="donation-notes" rows="3" class="dt-input">${escapeHtml(d.notes || '')}</textarea>
                    </div>
    ...
    `;}async function bindDonationFormHandlersRoute({userId,charities,existingDonation},deps){const{createDonationOnServer,createOrGetCharityOnServer,db,escapeHtml,isCharityCacheFresh,navigate,Sync,updateDonationOnServer,updateTotals,}=deps;const isEditMode=!!existingDonation;const charityInput=document.getElementById('donation-charity-input');const charityIdInput=document.getElementById('donation-charity-id');const charityEinInput=document.getElementById('donation-charity-ein');const suggestionsBox=document.getElementById('charity-suggestions');document.getElementById('btn-back-donations')?.addEventListener('click',()=>navigate('/donations'));const dateInput=document.getElementById('donation-date');if(dateInput&&!dateInput.value){dateInput.value=new Date().toISOString().split('T')[0];}if(isEditMode&&charityInput&&!charityInput.value&&existingDonation.charity_id){const c=charities.find(ch=>ch.id===existingDonation.charity_id);if(c)charityInput.value=c.name;}let suggestionTimer=null;charityInput?.addEventListener('input',(e)=>{charityIdInput.value='';charityEinInput.value='';const q=e.target.value.trim();if(suggestionTimer)clearTimeout(suggestionTimer);if(!q){suggestionsBox.innerHTML='';suggestionsBox.classList.add('hidden');return;}suggestionTimer=setTimeout(async()=>{try{const qLower=q.toLowerCase();let localMatches=[];try{localMatches=await db.charities.where('user_id').equals(userId).filter(c=>isCharityCacheFresh(c)&&(c.name||'').toLowerCase().includes(qLower)).toArray();}catch(le){console.warn('Local charity lookup failed',le);}let remote=[];try{const res=await fetch(`/api/charities/search?q=${encodeURIComponent(q)}`,{credentials:'include'});if(res.ok){const data=await res.json();remote=data.results||[];}}catch(re){console.warn('Remote charity search failed',re);}const seen=new Set();const merged=[];for(const c of localMatches){const key=((c.ein||'').trim()||(c.name||'').trim()).toLowerCase();if(!seen.has(key)){seen.add(key);merged.push({id:c.id,ein:c.ein||'',name:c.name,location:'',source:'local'});}}for(const r of remote){const key=((r.ein||'').trim()||(r.name||'').trim()).toLowerCase();if(!seen.has(key)){seen.add(key);merged.push({id:'',ein:r.ein||'',name:r.name,location:r.location||'',source:'remote'});}}if(merged.length===0){suggestionsBox.innerHTML='<div class="p-2 text-sm text-slate-500 dark:text-slate-400">No matches</div>';suggestionsBox.classList.remove('hidden');return;}suggestionsBox.innerHTML=merged.map(r=>`
                ...
                `).join('');suggestionsBox.classList.remove('hidden');suggestionsBox.querySelectorAll('div[data-id][data-ein][data-name]').forEach(el=>{el.addEventListener('click',()=>{charityInput.value=el.dataset.name||'';charityIdInput.value=el.dataset.id||'';charityEinInput.value=el.dataset.ein||'';suggestionsBox.classList.add('hidden');});});}catch(err){console.error('Charity search failed',err);suggestionsBox.classList.add('hidden');}},300);});document.getElementById('donation-page-form')?.addEventListener('submit',async(e)=>{e.preventDefault();if(!userId){alert('Please sign in again');return;}const date=document.getElementById('donation-date').value;const charity_name=charityInput.value.trim();const charity_id=charityIdInput.value.trim();const charity_ein=charityEinInput.value.trim();const notes=document.getElementById('donation-notes').value.trim();const category=document.getElementById('donation-category').value;const amount=parseFloat(document.getElementById('donation-amount').value)||0;const receiptFiles=Array.from((document.getElementById('donation-receipts').files||[]));if(!date||!charity_name){alert('Please provide date and charity name');return;}const year=new Date(date).getFullYear();try{let charityId=charity_id||(isEditMode?existingDonation.charity_id:'')||'';if(!charityId){const resp=await createOrGetCharityOnServer(charity_name,charity_ein||null);const charity=resp&&resp.charity?resp.charity:null;if(charity&&charity.id){charityId=charity.id;await db.charities.put({id:charity.id,user_id:userId,name:charity.name,ein:charity.ein||'',category:charity.category||null,status:charity.status||null,classification:charity.classification||null,nonprofit_type:charity.nonprofit_type||null,deductibility:charity.deductibility||null,street:charity.street||null,city:charity.city||null,state:charity.state||null,zip:charity.zip||null,cached_at:Date.now()});}}const payload={date,charity_name,charity_id:charityId||null,charity_ein:charity_ein||null,category,amount,notes:notes||null};let donation;const fallbackId=isEditMode?existingDonation.id:crypto.randomUUID();if(isEditMode){await updateDonationOnServer(existingDonation.id,payload);donation={...existingDonation,date,charity_id:charityId||existingDonation.charity_id,notes:notes||null,category,amount,sync_status:'synced'};await db.donations.put(donation);}else{try{const res=await createDonationOnServer(payload);const serverId=res&&res.id?res.id:fallbackId;donation={id:serverId,user_id:userId,year,date,charity_id:charityId||null,notes:notes||null,category,amount,sync_status:'synced'};await db.donations.put(donation);}catch{donation={id:fallbackId,user_id:userId,year,date,charity_id:charityId||null,notes:notes||null,category,amount,sync_status:'pending'};await Sync.queueAction('donations',donation,'create');}}if(receiptFiles.length>0&&donation.sync_status==='synced'){for(const file of receiptFiles){await uploadReceiptForDonationRoute(file,donation.id,deps);}}else if(receiptFiles.length>0){alert('Donation saved offline. Upload receipts after sync completes.');}await updateTotals();await navigate('/donations');}catch(err){console.error('Failed to save donation',err);alert('Failed to save donation');}});}export async function renderDonationNewRoute(deps){const root=document.getElementById('route-content')||document.getElementById('app');const userId=deps.getCurrentUserId();const charities=userId?await deps.db.charities.where('user_id').equals(userId).toArray():[];root.innerHTML=buildDonationFormHtmlRoute({title:'New Donation',desc:'Add a donation and optionally attach receipts.',submitLabel:'Save Donation',categoryPrefill:'money'},deps);await bindDonationFormHandlersRoute({userId,charities,existingDonation:null},deps);}export async function renderDonationEditRoute(donationId,deps){const userId=deps.getCurrentUserId();const charities=userId?await deps.db.charities.where('user_id').equals(userId).toArray():[];const existing=await deps.db.donations.get(donationId);if(!existing){alert('Donation not found');await deps.navigate('/donations');return;}const charityName=charities.find(c=>c.id===existing.charity_id)?.name||'';const root=document.getElementById('route-content')||document.getElementById('app');root.innerHTML=buildDonationFormHtmlRoute({title:'Edit Donation',desc:'Update the details for this donation.',submitLabel:'Save Changes',existing:{...existing,_charityName:charityName},},deps);await bindDonationFormHandlersRoute({userId,charities,existingDonation:existing},deps);}export async function renderReceiptPageRoute(donationId,deps){const root=document.getElementById('route-content')||document.getElementById('app');root.innerHTML=`
        <div class="mx-auto max-w-3xl space-y-5">
    ...
    `;document.getElementById('btn-back-donations')?.addEventListener('click',()=>deps.navigate('/donations'));document.getElementById('btn-upload-receipts')?.addEventListener('click',async()=>{const fileInput=document.getElementById('receipt-upload-input');const files=fileInput?Array.from(fileInput.files||[]):[];if(files.length===0){alert('Please select a file to upload.');return;}try{for(const file of files){await uploadReceiptForDonationRoute(file,donationId,deps);}fileInput.value='';await refreshAttachedListRoute(donationId,root,deps);}catch(err){console.error('Upload failed',err);alert('Failed to upload receipt');}});await refreshAttachedListRoute(donationId,root,deps);}async function refreshAttachedListRoute(donationId,container,deps){const normalizedId=(donationId&&String(donationId).trim())?String(donationId):null;if(!normalizedId)return;const attached=await deps.db.receipts.where('donation_id').equals(normalizedId).toArray();const attachedList=container.querySelector('#attached-list');if(!attachedList)return;attachedList.innerHTML=attached.length===0?'<div class="rounded-xl border border-slate-200 dark:border-slate-700 px-3 py-4 text-sm text-slate-500 dark:text-slate-400">No attached receipts.</div>':attached.map(r=>`
        ...
        `).join('');attachedList.querySelectorAll('.preview-receipt-btn').forEach(b=>{b.addEventListener('click',async(e)=>{if(!navigator.onLine){alert('Receipt preview requires an internet connection.');return;}const key=e.currentTarget.dataset.key;try{const downloadUrl=await getReceiptDownloadUrlRoute(key);window.open(downloadUrl,'_blank');}catch{alert('Preview failed');}});});}async function uploadReceiptForDonationRoute(file,donationId,deps){const uploadRes=await fetch('/api/receipts/upload',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({file_type:file.type})});if(!uploadRes.ok)throw new Error('Failed to request upload URL');const uploadData=await uploadRes.json();const putRes=await fetch(uploadData.upload_url,{method:'PUT',headers:{'Content-Type':file.type},body:file});if(!putRes.ok&&putRes.status!==200&&putRes.status!==204){throw new Error('Receipt upload failed');}const confirmRes=await fetch('/api/receipts/confirm',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({key:uploadData.key,file_name:file.name,content_type:file.type,size:file.size,donation_id:donationId})});if(!confirmRes.ok)throw new Error('Failed to confirm receipt');const body=await confirmRes.json();await deps.db.receipts.put({id:crypto.randomUUID(),key:uploadData.key,file_name:file.name,content_type:file.type,size:file.size,donation_id:donationId,uploaded_at:new Date().toISOString(),server_id:body&&body.id?body.id:null});}