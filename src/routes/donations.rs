        ...
        match record {
            Ok(rec) => {
                ...
                let resolved_charity_id = if let Some(cid) = charity_id {
                    ...
                    .await
                    {
                        ...
                        Ok(None) => {
                            ...
                            if let Err(e) =
                                crate::db::charities::create_charity(&state.db, &new_charity).await
                            {
                ...
                if let Err(e) = crate::db::donations::add_donation(&state.db, &new_donation).await {
                    ...
                    let _ = crate::db::audit::log_audit(
                        &state.db,
                        &audit_id,
                        &user.id,
                        "import",
...
fn validate_update_donation_request(req: &UpdateDonationRequest) -> Result<(), &'static str> {
    ...
    if matches!(normalize_category(&req.category).as_deref(), Some("money"))
        && req.amount.unwrap_or(0.0) <= 0.0
    {
        return Err("Money donations require a positive amount");
    }
...
pub async fn create_donation(
    ...
    let charity_id = if let Some(cid) = req
        .charity_id
        .as_deref()
        .map(str::trim)
    ...
    (
        StatusCode::CREATED,
        AxumJson(serde_json::json!({ "status": "created", "id": id, "charity_id": charity_id })),
    )
        .into_response()
}
...
pub async fn list_donations(
    ...
    if let Some(since_str) = params.since.clone() {
        if let Ok(since_dt) = DateTime::parse_from_rfc3339(&since_str) {
            match crate::db::donations::list_donations_since(
                &state.db,
                &user_id,
                since_dt.with_timezone(&Utc),
            )
            .await
            {
                Ok(donations) => {
                    return AxumJson(serde_json::json!({ "donations": donations })).into_response()
                }
                Err(e) => {
