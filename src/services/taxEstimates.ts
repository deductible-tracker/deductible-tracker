import { normalizeDonationCategory, parseAmount } from "./donationFigures";

export function isLikelyQualifiedCharity(charity: any): boolean {
  if (!charity) return false;
  const deductibility = String(charity.deductibility || "").toLowerCase();
  if (!deductibility) return true;
  return (
    deductibility.includes("deductible") ||
    deductibility.includes("pc") ||
    deductibility.includes("public charity")
  );
}

export function normalizeFilingStatus(status: any): "single" | "married_joint" | "married_separate" | "head_household" {
...
export async function calculateTaxEstimates(
  donations: any[],
  charities: any[],
  receipts: any[],
  profile: any = {}
  ...
  const marginalTaxRate =
    Number.isFinite(profile.marginal_tax_rate) &&
    profile.marginal_tax_rate >= 0 &&
    profile.marginal_tax_rate <= 1
  ...
  for (const donation of donations || []) {
    const year =
      ...
      (donation.date ? new Date(donation.date).getFullYear() : new Date().getFullYear());
  ...
  for (const [year, buckets] of years.entries()) {
    const cashTotal = buckets.cash.reduce((sum: number, donation: any) => sum + donation.amount, 0);
    const nonCashTotal = buckets.nonCash.reduce((sum: number, donation: any) => sum + donation.amount, 0);

    let deductibleCash = 0;
    let deductibleNonCash = 0;

    if (itemizeDeductions) {
      const cashCap = agi ? agi * 0.6 : Number.POSITIVE_INFINITY;
      const nonCashCap = agi ? agi * 0.3 : Number.POSITIVE_INFINITY;
