import db from "./db";
import { getCurrentUser, getCurrentUserId, setCurrentUser } from "./currentUser";
import { apiJson } from "./apiClient";
import { decryptData, ensureVaultKey } from "./crypto";
import {
...
function getPendingProfileStorageKey(userId: string) {
  return `${PENDING_PROFILE_KEY_PREFIX}${userId}`;
}

function normalizeProfileUpdate(profile: any) {
...
function loadPendingProfileUpdate(userId: string) {
...
function savePendingProfileUpdate(userId: string, profile: any) {
  if (!userId) return;
...
function clearPendingProfileUpdate(userId: string) {
  if (!userId) return;
  try {
...
function normalizeQueueAction(table: string, action: string) {
...
function normalizeQueueItemId(value: any) {
...
export const Sync = {
  ...
  async queueAction(table: string, item: any, action: string) {
  ...
  async queueProfileUpdate(userId: string, profile = getCurrentUser()) {
    if (!userId) return;
