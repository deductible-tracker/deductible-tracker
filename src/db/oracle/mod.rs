...
pub(crate) async fn delete_user_data(
    ...
    task::spawn_blocking(move || -> anyhow::Result<()> {
        ...
        conn.execute("DELETE FROM receipts WHERE donation_id IN (SELECT id FROM donations WHERE user_id = :1)", &[&user_id])?;
        conn.execute("DELETE FROM donations WHERE user_id = :1", &[&user_id])?;
        conn.execute("DELETE FROM charities WHERE user_id = :1", &[&user_id])?;
        conn.execute("DELETE FROM audit_logs WHERE user_id = :1", &[&user_id])?;
        conn.execute("DELETE FROM audit_revisions WHERE user_id = :1", &[&user_id])?;
