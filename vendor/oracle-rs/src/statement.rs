...
impl Statement {
    ...
    fn parse(&mut self) {
        let trimmed = Self::strip_leading_comments(&self.sql);
    ...
    fn strip_leading_comments(sql: &str) -> &str {
        ...
        loop {
            if let Some(after_comment) = remaining.strip_prefix("--") {
                remaining = match after_comment.find('\n') {
                    Some(pos) => after_comment[pos + 1..].trim_start(),
                    None => "",
                };
                continue;
            }
            ...
            if let Some(after_comment) = remaining.strip_prefix("/*") {
                remaining = match after_comment.find("*/") {
                    Some(pos) => after_comment[pos + 2..].trim_start(),
                    None => "",
                };
                continue;
            }
...
mod tests {
    ...
    #[test]
    fn test_statement_type_detection_with_leading_comments() {
        assert!(Statement::new("-- heading\nCREATE TABLE t (x NUMBER)").is_ddl());
        assert!(Statement::new("/* heading */\nSELECT * FROM dual").is_query());
    }
}
