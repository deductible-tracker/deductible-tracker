...
use crate::constants::{
    control_type, error_code, BindDirection, FetchOrientation, FunctionCode, MessageType,
...
impl OracleStream {
    ...
    async fn renegotiate_tls(self, server_name: &str, tls_config: &TlsConfig) -> Result<Self> {
        match self {
            OracleStream::Plain(_) => Err(Error::FeatureNotSupported(
                "TLS renegotiation requested for a plain TCP stream".to_string(),
            )),
            OracleStream::Tls(stream) => stream
                .renegotiate(server_name, tls_config)
                .await
                .map(OracleStream::Tls),
        }
    }
}
...
impl ConnectionInner {
    ...
    async fn receive(&mut self) -> Result<bytes::Bytes> {
        loop {
                ...
                let Some(stream) = self.stream.as_mut() else {
                    return Err(
                        self.take_pending_error()
                            .map_or(Error::ConnectionClosed, Self::oracle_error_from_code),
                    );
                };
            ...
            let mut full_packet = header_buf;
            full_packet.extend(payload_buf);

            if full_packet.get(4) == Some(&(PacketType::Control as u8)) {
                self.process_control_packet(&full_packet)?;
                continue;
            }

            return Ok(bytes::Bytes::from(full_packet));
        }
    }

    fn process_control_packet(&mut self, packet: &[u8]) -> Result<()> {
        if packet.len() < PACKET_HEADER_SIZE + 2 {
            return Err(Error::Protocol("CONTROL packet too short".to_string()));
        }

        let control_type_value =
            u16::from_be_bytes([packet[PACKET_HEADER_SIZE], packet[PACKET_HEADER_SIZE + 1]]);

        match control_type_value {
            control_type::RESET_OOB => {
                self.server_info.supports_oob = false;
            }
            control_type::INBAND_NOTIFICATION => {
                if packet.len() < PACKET_HEADER_SIZE + 10 {
                    return Err(Error::Protocol(
                        "INBAND_NOTIFICATION packet too short".to_string(),
                    ));
                }

                self.pending_error_num = u32::from_be_bytes([
                    packet[PACKET_HEADER_SIZE + 6],
                    packet[PACKET_HEADER_SIZE + 7],
                    packet[PACKET_HEADER_SIZE + 8],
                    packet[PACKET_HEADER_SIZE + 9],
                ]);
            }
            _ => {}
        }
    ...
    fn map_read_error(&mut self, err: std::io::Error) -> Error {
        if matches!(err.kind(), std::io::ErrorKind::UnexpectedEof | std::io::ErrorKind::ConnectionReset)
        {
            if let Some(code) = self.take_pending_error() {
                if code != 0 && code != error_code::SESSION_SHUTDOWN && code != error_code::INBAND_MESSAGE {
                    return Self::oracle_error_from_code(code);
                }
            }
        }
    ...
    fn take_pending_error(&mut self) -> Option<u32> {
        if self.pending_error_num == 0 {
            None
        } else {
            Some(std::mem::take(&mut self.pending_error_num))
        }
    }

    fn oracle_error_from_code(code: u32) -> Error {
        Error::OracleError {
            code,
    ...
    /// Receive a complete response starting from a DATA packet that has already been read.
    async fn receive_response_from_packet(&mut self, first_packet: bytes::Bytes) -> Result<bytes::Bytes> {
        use crate::constants::{data_flags, MessageType};
        ...
        loop {
            let packet = match current_packet.take() {
                Some(packet) => packet,
    ...
    async fn send_marker(&mut self, marker_type: u8) -> Result<()> {
        ...
        // Marker packets use the same large-SDU framing rules as other packets.
...
impl Connection {
    ...
    async fn connect_stream(config: &Config) -> Result<OracleStream> {
        ...
        if config.is_tls_enabled() {
            ...
            if tls_config.wallet_path.is_some() && tls_config.server_name.is_none() {
                if let Some(oracle_sni_name) = config.oracle_tcps_sni_name() {
                    tls_config.server_name = Some(oracle_sni_name);
                    tls_config.verify_server_hostname = false;
                }
            }
            let tls_stream = connect_tls(tcp_stream, &config.host, &tls_config).await?;
            Ok(OracleStream::Tls(tls_stream))
        } else {
            Ok(OracleStream::Plain(tcp_stream))
        }
    }

    fn build_connect_packets(
        config: &Config,
        connect_data: Option<&str>,
    ) -> Result<(Bytes, Option<Bytes>)> {
        let mut connect_msg = ConnectMessage::from_config(config);
        if let Some(connect_data) = connect_data {
            connect_msg.connect_data = connect_data.to_string();
        }
        connect_msg.build_with_continuation()
    }
    ...
    async fn perform_handshake(&self) -> Result<()> {
        ...
        self.send_connect_packet()
            .await
            .map_err(|e| Error::Internal(format!("CONNECT/ACCEPT exchange failed: {}", e)))?;
        ...
        if needs_oob_check {
            self.send_oob_check()
                .await
                .map_err(|e| Error::Internal(format!("OOB check failed: {}", e)))?;
        }

        // Step 3: Protocol negotiation
        self.negotiate_protocol()
            .await
            .map_err(|e| Error::Internal(format!("protocol negotiation failed: {}", e)))?;

        // Step 4: Data types negotiation
        self.negotiate_data_types()
            .await
            .map_err(|e| Error::Internal(format!("data types negotiation failed: {}", e)))?;

        // Step 5: Authentication
        self.authenticate()
            .await
            .map_err(|e| Error::Internal(format!("authentication failed: {}", e)))?;
    ...
    async fn send_connect_packet(&self) -> Result<()> {
        ...
        let (mut connect_packet, mut continuation) =
            Self::build_connect_packets(&active_config, None)?;
        ...
        loop {
            ...
            match packet_type {
                2 => {
                    ...
                    inner.server_info.supports_oob =
                        accept.supports_oob && !active_config.is_tls_enabled();
                ...
                4 => {
                    ...
                    return Err(refuse.into_error(match &active_config.service {
                        ServiceMethod::ServiceName(service_name) => Some(service_name.as_str()),
                        ServiceMethod::Sid(sid) => Some(sid.as_str()),
                    }));
                }
                5 => {
                    let packet = Packet::from_bytes(response)?;
                    ...
                    if redirect_count > MAX_REDIRECTS {
                        return Err(Error::ProtocolError(
                            "Server redirected too many times during connect".to_string(),
                        ));
                    }
                    ...
                    let redirect_host = redirect
                        .host
                        .clone()
                        .ok_or_else(|| Error::ConnectionRedirect(redirect.address.clone()))?;

                    let mut redirect_config = active_config.clone();
                    redirect_config.host = redirect_host;
                    if let Some(port) = redirect.port {
                        redirect_config.port = port;
                    }

                    inner.stream = Some(Self::connect_stream(&redirect_config).await?);
                    inner.large_sdu = false;
                    inner.sdu_size = 8192;

                    let (next_connect_packet, next_continuation) = Self::build_connect_packets(
                        &redirect_config,
                        redirect.connect_string.as_deref(),
                    )?;
                    connect_packet = next_connect_packet;
                    continuation = next_continuation;
                    active_config = redirect_config;
                    resend_count = 0;
                ...
                11 => {
                    ...
                    let requires_tls_renegotiation = active_config.is_tls_enabled()
                        && response
                            .get(5)
                            .map(|flags| {
                                (flags & crate::constants::packet_flags::TLS_RENEG) != 0
                            })
                            .unwrap_or(false);

                    if requires_tls_renegotiation {
                        let tls_config = active_config
                            .tls_config
                            .as_ref()
                            .cloned()
                            .ok_or_else(|| {
                                Error::Internal(
                                    "TLS renegotiation requested without TLS configuration"
                                        .to_string(),
                                )
                            })?;
                        let stream = inner.stream.take().ok_or_else(|| {
                            Error::Internal(
                                "TLS renegotiation requested without an active stream"
                                    .to_string(),
                            )
                        })?;
                        inner.stream = Some(stream.renegotiate_tls(&active_config.host, &tls_config).await?);
                    }
                ...
                match inner.receive().await {
                    Ok(pkt) => {
                        ...
                        if pkt_type == PacketType::Marker as u8 {
                            ...
                            return inner.receive_response_from_packet(pkt).await;
                        } else {
                            break;
                        }
                    }
    ...
    fn parse_return_parameters_internal(
            ...
            ($label:literal, $expr:expr) => {{
                ...
                ($expr).map_err(|err| {
                    Error::Protocol(format!(
                        "failed parsing return parameters field '{}' at offset {}: {}",
                        $label, pos, err
                    ))
                })?
            }};
        ...
        let num_params = parse_field!("al8o4l_count", buf.read_ub2());
        for _ in 0..num_params {
            parse_field!("al8o4l_entry", buf.skip_ub4());
        }

        let al8txl = parse_field!("al8txl", buf.read_ub2());
        if al8txl > 0 {
            parse_field!("al8txl_payload", buf.skip(al8txl as usize));
        }

        // num key/value pairs - skip for now
        let num_pairs = parse_field!("key_value_pair_count", buf.read_ub2());
        for _ in 0..num_pairs {
            let _ = parse_field!("key_value_text", buf.read_bytes_with_length());
            let _ = parse_field!("key_value_binary", buf.read_bytes_with_length());
            parse_field!("key_value_keyword", buf.skip_ub2());
        }

        // registration
        let num_bytes = parse_field!("registration_length", buf.read_ub2());
        if num_bytes > 0 {
            parse_field!("registration_payload", buf.skip(num_bytes as usize));
        }

        // If arraydmlrowcounts was requested, parse the row counts
        if want_row_counts && buf.remaining() >= 4 {
            let num_rows = parse_field!("array_dml_rowcount_count", buf.read_ub4()) as usize;
            let mut row_counts = Vec::with_capacity(num_rows);
            for _ in 0..num_rows {
                let count = parse_field!("array_dml_rowcount", buf.read_ub8());
    ...
    fn parse_server_side_piggyback(&self, buf: &mut ReadBuffer) -> Result<()> {
        macro_rules! parse_field {
            ($opcode:expr, $label:literal, $expr:expr) => {{
                let pos = buf.position();
                ($expr).map_err(|err| {
                    Error::Protocol(format!(
                        "failed parsing server-side piggyback opcode {} field '{}' at offset {}: {}",
                        $opcode, $label, pos, err
                    ))
                })?
            }};
        }
        ...
        match opcode {
            // TNS_SERVER_PIGGYBACK_QUERY_CACHE_INVALIDATION | TRACE_EVENT
            1 | 3 => {}

            // TNS_SERVER_PIGGYBACK_OS_PID_MTS
            2 => {
                let _ = parse_field!(opcode, "os_pid_mts_temp16", buf.read_ub2());
                parse_field!(opcode, "os_pid_mts_payload", buf.skip_raw_bytes_chunked());
            }

            // TNS_SERVER_PIGGYBACK_SESS_RET
            4 => {
                parse_field!(opcode, "sess_ret_dty_count", buf.skip_ub2());
                parse_field!(opcode, "sess_ret_dty_length", buf.skip_ub1());
                let num_elements = parse_field!(opcode, "sess_ret_num_elements", buf.read_ub2());
                if num_elements > 0 {
                    parse_field!(opcode, "sess_ret_num_elements_length", buf.skip_ub1());
                    self.skip_keyword_value_pairs(buf, num_elements).map_err(|err| {
                        Error::Protocol(format!(
                            "failed parsing server-side piggyback opcode {} field 'sess_ret_pairs' at offset {}: {}",
                            opcode,
                            buf.position(),
                            err
                        ))
                ...
                let _flags = parse_field!(opcode, "sess_ret_session_flags", buf.read_ub4());
                let _session_id = parse_field!(opcode, "sess_ret_session_id", buf.read_ub4());
                let _serial_num = parse_field!(opcode, "sess_ret_serial_num", buf.read_ub2());
            }

            // TNS_SERVER_PIGGYBACK_SYNC
            5 => {
                parse_field!(opcode, "sync_dty_count", buf.skip_ub2());
                parse_field!(opcode, "sync_dty_length", buf.skip_ub1());
                let num_elements = parse_field!(opcode, "sync_num_elements", buf.read_ub2());
                if num_elements > 0 {
                    parse_field!(opcode, "sync_num_elements_length", buf.skip_ub1());
                    self.skip_keyword_value_pairs(buf, num_elements).map_err(|err| {
                        Error::Protocol(format!(
                            "failed parsing server-side piggyback opcode {} field 'sync_pairs' at offset {}: {}",
                            opcode,
                            buf.position(),
                            err
                        ))
                ...
                parse_field!(opcode, "sync_overall_flags", buf.skip_ub4());
            }

            // TNS_SERVER_PIGGYBACK_LTXID
            7 => {
                let _ = parse_field!(opcode, "ltxid", buf.read_bytes_with_length());
            }

            // TNS_SERVER_PIGGYBACK_AC_REPLAY_CONTEXT
            8 => {
                parse_field!(opcode, "ac_replay_dty_count", buf.skip_ub2());
                parse_field!(opcode, "ac_replay_dty_length", buf.skip_ub1());
                parse_field!(opcode, "ac_replay_flags", buf.skip_ub4());
                parse_field!(opcode, "ac_replay_error_code", buf.skip_ub4());
                parse_field!(opcode, "ac_replay_queue", buf.skip_ub1());
                let _ = parse_field!(opcode, "ac_replay_context", buf.read_bytes_with_length());
            }

            // TNS_SERVER_PIGGYBACK_EXT_SYNC
            9 => {
                parse_field!(opcode, "ext_sync_dty_count", buf.skip_ub2());
                parse_field!(opcode, "ext_sync_dty_length", buf.skip_ub1());
            }

            // TNS_SERVER_PIGGYBACK_SESS_SIGNATURE
            10 => {
                parse_field!(opcode, "sess_signature_dty_count", buf.skip_ub2());
                parse_field!(opcode, "sess_signature_dty_length", buf.skip_ub1());
                parse_field!(opcode, "sess_signature_flags", buf.skip_ub8());
                parse_field!(opcode, "sess_signature_client", buf.skip_ub8());
                parse_field!(opcode, "sess_signature_server", buf.skip_ub8());
            }
    ...
    fn parse_dml_response(&self, payload: &[u8], ttc_field_version: u8) -> Result<QueryResult> {
        ...
        while !end_of_response && buf.remaining() > 0 {
            ...
            match msg_type {
                ...
                // Parameter (8) - return parameters
                x if x == MessageType::Parameter as u8 => {
                    self.parse_return_parameters(&mut buf).map_err(|err| {
                        Error::Protocol(format!(
                            "failed parsing DML parameter message after message sequence {:?}: {}",
                            message_history, err
                        ))
                    })?;
                }

                // Server-side piggyback (23)
                23 => {
                    self.parse_server_side_piggyback(&mut buf).map_err(|err| {
                        Error::Protocol(format!(
                            "failed parsing DML server-side piggyback after message sequence {:?}: {}",
                            message_history, err
                        ))
                    })?;
                }
                ...
                _ => {
                    return Err(Error::Protocol(format!(
                        "unhandled DML response message type {} at offset {} after message sequence {:?}",
                        msg_type,
    ...
    fn parse_error_info_with_rowcount(
        ...
        macro_rules! parse_field {
            ($label:literal, $expr:expr) => {{
                ...
                ($expr).map_err(|err| {
                    Error::Protocol(format!(
                        "failed parsing error info field '{}' at offset {} with ttc_field_version {}: {}",
                        $label, pos, ttc_field_version, err
                    ))
                })?
            }};
        }

        let _call_status = parse_field!("call_status", buf.read_ub4());
        parse_field!("end_to_end_seq", buf.skip_ub2());
        parse_field!("current_row_number", buf.skip_ub4());
        parse_field!("error_number_short", buf.skip_ub2());
        parse_field!("array_elem_error_1", buf.skip_ub2());
        parse_field!("array_elem_error_2", buf.skip_ub2());
        let cursor_id = parse_field!("cursor_id", buf.read_ub2());
        let _error_pos = parse_field!("error_position", buf.read_sb2());
        parse_field!("sql_type_legacy", buf.skip_ub1());
        parse_field!("fatal_flag", buf.skip_ub1());
        parse_field!("flags", buf.skip_ub1());
        parse_field!("user_cursor_options", buf.skip_ub1());
        parse_field!("upi_parameter", buf.skip_ub1());
        let _flags = parse_field!("flags_second", buf.read_ub1());
        parse_field!("rowid_rba", buf.skip_ub4());
        parse_field!("rowid_partition_id", buf.skip_ub2());
        parse_field!("rowid_skip", buf.skip_ub1());
        parse_field!("rowid_block_num", buf.skip_ub4());
        parse_field!("rowid_slot_num", buf.skip_ub2());
        parse_field!("os_error", buf.skip_ub4());
        parse_field!("statement_number", buf.skip_ub1());
        parse_field!("call_number", buf.skip_ub1());
        parse_field!("padding", buf.skip_ub2());
        parse_field!("success_iters", buf.skip_ub4());
        parse_field!("logical_rowid", buf.skip_bytes_with_ub4_length());

        let num_batch_errors = parse_field!("batch_error_count", buf.read_ub2());
        if num_batch_errors > 0 {
            let first_byte = parse_field!("batch_error_first_byte", buf.read_ub1());
            for _ in 0..num_batch_errors {
                if first_byte == crate::constants::length::LONG_INDICATOR {
                    parse_field!("batch_error_chunk_length", buf.skip_ub4());
                }
                parse_field!("batch_error_code", buf.skip_ub2());
            }
            if first_byte == crate::constants::length::LONG_INDICATOR {
                parse_field!("batch_error_end_marker", buf.skip(1));
            }
        }

        let num_offsets = parse_field!("batch_error_offset_count", buf.read_ub4());
        if num_offsets > 0 {
            let first_byte = parse_field!("batch_error_offset_first_byte", buf.read_ub1());
            for _ in 0..num_offsets {
                if first_byte == crate::constants::length::LONG_INDICATOR {
                    parse_field!("batch_error_offset_chunk_length", buf.skip_ub4());
                }
                parse_field!("batch_error_offset", buf.skip_ub4());
            }
            if first_byte == crate::constants::length::LONG_INDICATOR {
                parse_field!("batch_error_offset_end_marker", buf.skip(1));
            }
        }

        let num_batch_msgs = parse_field!("batch_error_message_count", buf.read_ub2());
        if num_batch_msgs > 0 {
            parse_field!("batch_error_message_packed_size", buf.skip_ub1());
            for _ in 0..num_batch_msgs {
                parse_field!("batch_error_message_chunk_length", buf.skip_ub2());
                let _ = parse_field!("batch_error_message", buf.read_string_with_length());
                parse_field!("batch_error_message_end_marker", buf.skip(2));
            }
        }

        let error_code = parse_field!("extended_error_number", buf.read_ub4());
        let row_count = parse_field!("row_count", buf.read_ub8());

        if ttc_field_version >= crate::constants::ccap_value::FIELD_VERSION_21_1 {
            parse_field!("sql_type_21_1", buf.skip_ub4());
            parse_field!("server_checksum", buf.skip_ub4());
        }

        let error_msg = if error_code != 0 {
            parse_field!("error_message", buf.read_string_with_length()).map(|s| s.trim().to_string())
        } else {
            None
        };
    ...
    async fn send_marker(&self, inner: &mut ConnectionInner, marker_type: u8) -> Result<()> {
        ...
        // Build MARKER packet header using the negotiated SDU framing.
