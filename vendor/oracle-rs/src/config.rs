...
impl Config {
    ...
    pub fn build_connect_string(&self) -> String {
        ...
        let use_sni = self.tls_mode == TlsMode::Require
            && self
                .tls_config
                .as_ref()
                .and_then(|config| config.server_name.as_ref())
                .is_some();

        if use_sni {
            parts.push("(USE_SNI=ON)".to_string());
        }

        // Oracle thin-mode transports negotiate TLS outside the connect descriptor.
        let protocol = "TCP";
        parts.push(format!(
        ...
        parts.push(build_connection_identifier());
    ...
    /// Build Oracle's special TCPS SNI token for service-name connections.
    pub fn oracle_tcps_sni_name(&self) -> Option<String> {
        self.service.service_name().map(|service_name| {
            format!(
                "S{}.{}.V3.{}",
                service_name.len(),
                service_name,
                crate::constants::version::DESIRED
            )
        })
    }
}
...
fn build_connection_identifier() -> String {
    let program = sanitize_descriptor_value(
        std::env::args()
            .next()
            .as_deref()
    ...
    let host = sanitize_descriptor_value(
        hostname::get()
            .ok()
    ...
    let user = sanitize_descriptor_value(
        std::env::var("USER")
            .ok()
    ...
    format!("(CID=(PROGRAM={program})(HOST={host})(USER={user}))")
}

fn sanitize_descriptor_value(value: &str) -> String {
    let sanitized = value
        .chars()
        .map(|ch| match ch {
            '(' | ')' | '=' => '_',
